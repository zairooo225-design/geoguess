...
import { Volume2, VolumeX, Lock, Loader2 } from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    if (gameState === "PLAYING") {
      if (!roundStats && !showPreview) {
        audioManager.playBgm(0.3, false, -1);
      }
    }
  ...
  const handleStartGame = (settings: { title: string; className: string; nameProtected: boolean; numQuestions: number; difficulty: string; globalVolume?: number; isSolo?: boolean }) => {
    ...
    setGameState("LOADING_ROOM");
    setTimeout(() => {
      socket.emit("create-room", { ...settings, isSolo: isSoloMode });
    }, 2000);
  };
              ...
              <div className="text-center">
                ...
                <div className="flex flex-col sm:flex-row gap-3 justify-center items-center">
                  <button 
                    ...
                    className="btn-shine bg-transparent border-2 border-[#2D2D2D] text-[#2D2D2D] font-black px-8 py-3 rounded-lg uppercase tracking-widest hover:bg-[#2D2D2D] hover:text-white transition-colors"
                  >
                    Teacher Login
                  </button>
                  <button 
                    ...
                    className="btn-shine bg-transparent border-2 border-[#2D2D2D] text-[#2D2D2D] font-black px-8 py-3 rounded-lg uppercase tracking-widest hover:bg-[#3EACA8] hover:text-white hover:border-[#3EACA8] transition-colors"
                  >
                    Solo Play
                  </button>
                </div>
                   ...
                   <AnimatePresence>
                     {players.map((p) => (
                       <motion.div 
                         ...
                         className="bg-[#D4A373] text-[#2D2D2D] border-2 border-[#2D2D2D] shadow-[2px_2px_0px_#2D2D2D] font-black text-xl px-4 py-3 rounded-lg uppercase whitespace-nowrap cursor-default flex items-center gap-2"
                       >
                         {p.avatar && <img src={p.avatar} alt="" className="w-8 h-8 rounded-full border-2 border-[#2D2D2D] object-cover" />}
                         {p.name}
                       </motion.div>
                     ))}
                   </AnimatePresence>
               ...
               {isHost ? (
                 ...
                 <div className="w-full max-w-4xl grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 px-4 overflow-y-auto pb-24">
                    {players.filter(p => p.id !== socket.id).map(p => {
                      const isMyTeammate = players.find(me => me.id === socket.id)?.teammateId === p.id;
                      ...
                      return (
                        ...
                        >
                           {isMyTeammate && (
                             <div className="absolute top-0 right-0 bg-[#5CB85C] text-white font-bold px-2 py-1 text-xs rounded-bl-lg">
                               TEAM
                             </div>
                           )}
