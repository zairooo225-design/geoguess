...
// Default world center coordinates to avoid re-renders resetting map view
const DEFAULT_CENTER = { lat: 20, lng: 0 }; 
const DEFAULT_ZOOM = 2;

// Ensure the map resizes nicely and centers initially or when targetCountry changes
const MapUpdater = ({ center, zoom, resetTrigger }: { center: { lat: number; lng: number }; zoom: number; resetTrigger: any }) => {
  const map = useMap();
  useEffect(() => {
    map.setView([center.lat, center.lng], zoom, { animate: false });
    map.invalidateSize();
  }, [resetTrigger, map]); // Only re-view when the targetCountry resets (e.g., transition to new round)
  return null;
};
  ...
  return (
    <div className="relative w-full flex-1 min-h-0 h-full rounded-xl overflow-hidden border-4 border-[#2D2D2D] shadow-[8px_8px_0px_#2D2D2D] bg-[#AEC6C2]">
      {loading ? (
        ...
        <MapContainer center={[DEFAULT_CENTER.lat, DEFAULT_CENTER.lng]} zoom={DEFAULT_ZOOM} scrollWheelZoom={true} className="w-full h-full cursor-crosshair">
          <MapUpdater center={DEFAULT_CENTER} zoom={DEFAULT_ZOOM} resetTrigger={targetCountry} />
          {/* Base Map Tiles */}
          <TileLayer
      ...
      {!disabled && selectedGeo && (
         ...
         >
           <button
             ...
             className="w-full max-w-sm bg-[#C16757] text-white border-4 border-[#2D2D2D] shadow-[4px_4px_0_#2D2D2D] py-3 rounded-xl font-black text-xl hover:translate-y-1 hover:translate-x-1 hover:shadow-none transition-all uppercase tracking-widest"
           >
             Confirm Pick
           </button>
         </motion.div>
