...
io.on("connection", (socket) => {
  ...
  socket.on("send-team-request", ({ roomCode, targetPlayerId }) => {
     ...
     // basic team request handling (simplified mapping: just set it)
...
app.post("/api/validate-name", async (req, res) => {
  ...
  const blocklist = ["porn", "sex", "p3n1s", "dick", "cock", "pussy", "vagina", "boob", "tits", "nigger", "nigga", "poop", "butt", "fart", "stupid", "idiot", "dumb", "crap", "shit", "fuck", "bitch", "cunt", "slut", "whore"];
  ...
  try {
    const prompt = `You are a strict automated profanity and safety filter for a children's classroom game.
Evaluate this username: "${name}".
If it contains ANY of the following, reply ONLY with "INAPPROPRIATE":
- Profanity, dirty words, or circumvented swear words (e.g. p0rn, a$$, fck)
